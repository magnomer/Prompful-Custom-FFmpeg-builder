...
func LReleaseLineGet(releaseLineKey string) (LReleaseSupport, bool) {
	version, found := LReleaseRecommendedForLineGet(releaseLineKey)
	if !found {
		return LReleaseSupport{}, false
	}
	return LReleaseSupportForVersionGet(version)
}

func LReleaseSupportResolve(version string) (LReleaseSupport, bool) {
	lineKey := LReleaseLineKeyGet(version)
	if lineKey == "" {
		return LReleaseSupport{}, false
	}
	if recommended, found := LReleaseRecommendedForLineGet(lineKey); found {
		return LReleaseSupportForVersionGet(recommended)
	}
	chosenMajor, chosenMinor, ok := LReleaseLineSplit(lineKey)
	if !ok {
		return LReleaseSupport{}, false
	}
	highestKey := LReleaseLineHighestGet()
...
func LReleaseSupportForVersionGet(ffmpegVersion string) (LReleaseSupport, bool) {
	...
	release := LReleaseSupport{Libraries: map[string]LLibrarySupport{}, Options: LCatalogSupportedOptionIdsRead(versionRecord)}
	for libraryId, libraryRecord := range resolver.LibraryRecords {
		...
		if !LCatalogBoolField(versionObject, "supportedByFfmpeg") {
			continue
		}
		release.Libraries[libraryId] = LLibrarySupport{
			MinVersion:  LCatalogStringField(versionObject, "ffmpegPkgConfigMinimumVersion"),
			Unavailable: !LCatalogBoolField(versionObject, "availableInCurrentV4"),
			SourceBuild: LCatalogBoolField(versionObject, "sourceBuildRequiredByReleaseManifest"),
		}
	}
	return release, true
}

func LCatalogSupportedOptionIdsRead(versionRecord map[string]any) []string {
	...
	for _, item := range items {
		...
		optionId := LCatalogStringField(itemObject, "optionId")
		if optionId != "" {
			optionIds = append(optionIds, optionId)
		}
	}
	return LCatalogStringsUniqueSortedStable(optionIds)
}
...
func LReleaseRecommendedForLineGet(releaseLineKey string) (string, bool) {
	resolver, _, err := LCatalogResolverLoad()
	if err != nil {
		return "", false
	}
...
func LReleaseLineHighestGet() string {
	best := ""
	bestMajor, bestMinor := -1, -1
	for _, lineKey := range LReleaseLineListGet() {
...
func LReleaseLineListGet() []string {
	resolver, _, err := LCatalogResolverLoad()
	if err != nil {
		return nil
	}
	...
	return LCatalogStringsUniqueSortedStable(lineKeys)
}

func LReleaseLineKeyGet(version string) string {
...
func LVersionSemverCompare(left string, right string) int {
	...
	for index := 0; index < len(leftParts) || index < len(rightParts); index++ {
		leftValue := LVersionComponentNumericGet(leftParts, index)
		rightValue := LVersionComponentNumericGet(rightParts, index)
...
func LVersionComponentNumericGet(parts []string, index int) int {
	if index >= len(parts) {
		return 0
	}
...
func LReleaseRequiredGet(lineKey string) (string, error) {
	version, found := LReleaseRecommendedForLineGet(lineKey)
	if !found {
