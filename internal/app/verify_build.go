...
// libraryProbeComponents maps a flagless library (no --enable-* flag to find in
...
var libraryProbeComponents = map[string][]string{
	"png": {"png", "apng"},
}

// LibraryVerification reports whether one planned library is present in the built
// ffmpeg, verified either by its configure flag or by probing for a component.
type LibraryVerification struct {
...
// BuildVerification is the Method A result: compare the build plan's libraries
// against the configuration string the built ffmpeg.exe reports for itself.
type BuildVerification struct {
	FfmpegPath            string                `json:"ffmpegPath"`
	FfmpegVersion         string                `json:"ffmpegVersion"`
	Libraries             []LibraryVerification `json:"libraries"`
...
// VerifyBuildResult runs the built ffmpeg.exe, reads the configure flags it was
// compiled with, and checks each planned library's enable flag is present.
func (app *App) VerifyBuildResult(workspaceDirectory string) (BuildVerification, error) {
	if workspaceDirectory == "" {
		return BuildVerification{}, errors.New("workspace directory is required")
	}
	layout := workspace.WorkspaceLayoutFor(workspaceDirectory)
	if err := workspace.CheckRealPathInsideWorkspace(layout.WorkspaceDirectory, layout.ArtifactsDirectory); err != nil {
		return BuildVerification{}, err
	}
	ffmpegPath := filepath.Join(layout.ArtifactsDirectory, "ffmpeg.exe")
	if err := workspace.CheckRealPathInsideWorkspace(layout.WorkspaceDirectory, ffmpegPath); err != nil {
		return BuildVerification{}, err
	}
	...
	versionOutput, err := runFfmpegVersion(ffmpegPath)
	if err != nil {
		verification.Overall = "unverifiable"
		verification.Message = "Could not run ffmpeg.exe to read its build configuration: " + err.Error()
		return verification, nil
	}
	verification.FfmpegVersion = parseFfmpegVersion(versionOutput)
	configuredFlags := parseConfiguredFlags(versionOutput)
	...
	_, report, reportErr := readLatestArtifactReport(layout)
	if reportErr != nil {
		verification.Overall = "unverifiable"
	...
	ensureComponentNames := func() map[string]bool {
		if !componentNamesLoaded {
			componentNames = probeFfmpegComponents(ffmpegPath)
	...
	for _, library := range report.SelectedLibraries {
		...
		libraryVerification := LibraryVerification{
		...
		probeComponents := libraryProbeComponents[library.LibraryId]
	...
	// "Extra" = a known catalog library's enable flag present in the binary but
	// not planned. Restricting to catalog flags avoids flagging the many core
	// --enable-* options (gpl, vulkan, sdl2, ...) that are not libraries.
	knownLibraryFlags := catalogLibraryFlags()
...
// catalogLibraryFlags returns every --enable-* flag known to belong to a
// catalog library, used to tell real "extra" libraries apart from core flags.
// Library configure flags do not vary by shell profile, so the default is fine.
func catalogLibraryFlags() map[string]bool {
	flags := map[string]bool{}
	for _, library := range planning.LibraryCatalogForShellProfile(planning.DefaultFfmpegBuildSettings().WindowsShellProfileName) {
		for _, flag := range library.ConfigureFlags {
			flag = strings.TrimSpace(flag)
			if strings.HasPrefix(flag, "--enable-") {
				flags[flag] = true
...
// probeFfmpegComponents runs the built ffmpeg and collects the names of every
...
func probeFfmpegComponents(ffmpegPath string) map[string]bool {
	names := map[string]bool{}
	for _, listArg := range []string{"-decoders", "-encoders", "-demuxers", "-muxers"} {
		output, err := runFfmpegList(ffmpegPath, listArg)
		if err != nil {
			continue
		}
		addComponentNames(names, output)
	}
	return names
}

func runFfmpegList(ffmpegPath string, listArg string) (string, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 20*time.Second)
	defer cancel()
	output, err := exec.CommandContext(ctx, ffmpegPath, "-hide_banner", listArg).CombinedOutput()
	if err != nil {
		return "", err
	}
...
func addComponentNames(set map[string]bool, listOutput string) {
...
func runFfmpegVersion(ffmpegPath string) (string, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 20*time.Second)
	defer cancel()
	output, err := exec.CommandContext(ctx, ffmpegPath, "-hide_banner", "-version").CombinedOutput()
	if err != nil {
		return "", err
	}
	return string(output), nil
...
func parseConfiguredFlags(versionOutput string) map[string]bool {
	flags := map[string]bool{}
