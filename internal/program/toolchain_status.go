...
// LStatusToolchain is the fast, disk-based answer to "is this workspace already
// prepared?". The presence of usr/bin/bash.exe is authoritative; the release-support manifest
// only enriches it with metadata and may be absent for installs made before the
// release-support manifest existed.
type LStatusToolchain struct {
...
// LVerificationToolchain is the deep, on-demand check: it queries the private
...
// LManifestToolchainWrite records the successful install metadata. It is
...
// release-support manifest read.
func (program *LProgram) LStatusToolchainGet(workspaceDirectory string, windowsShellProfileName string) (LStatusToolchain, error) {
	status := LStatusToolchain{PackageNames: []string{}}
	if workspaceDirectory == "" {
		return status, nil
	}
	...
	if err != nil {
		// Installed without a release-support manifest (older install). Still report it as present.
		return status, nil
	}
...
func (program *LProgram) LProfileToolchainList(workspaceDirectory string) ([]LStatusToolchain, error) {
	...
	for _, profileName := range LProfileShellSupportedNames {
		...
		// if an older install has no release-support manifest to read the name from.
...
// LToolchainEnvironmentClear removes every program-managed private build environment in
...
func (program *LProgram) LToolchainEnvironmentClear(workspaceDirectory string) error {
	if workspaceDirectory == "" {
		return errors.New("workspace directory is empty")
	}
	workspaceLayout := workspace.LWorkspaceLayoutResolve(workspaceDirectory)
	if err := workspace.LPathWorkspaceCheck(workspaceDirectory, workspaceLayout.ToolchainsDirectory); err != nil {
		return err
	}
	...
	if err := workspace.LPathRealCheck(workspaceDirectory, workspaceLayout.ToolchainsDirectory); err != nil {
		return err
	}
	return os.RemoveAll(workspaceLayout.ToolchainsDirectory)
}

// LToolchainInstallVerify deeply checks the private MSYS2 install by querying
...
func (program *LProgram) LToolchainInstallVerify(workspaceDirectory string, windowsShellProfileName string) (LVerificationToolchain, error) {
	verification := LVerificationToolchain{MissingPackageNames: []string{}}
	if workspaceDirectory == "" {
